...
The code example uses two Capture and Compare Unit 4 (CCU4) slices of the XMC MCU:
...
The application uses two CCU4 slices configured using the CCU4 personality:
   ...
   1. PWM_0 CCU4 slice configured in compare mode. It will generate a PWM signal with a frequency of 2 Hz and 75% duty cycle.

      **Figure 2. PWM signal**

      ![](images/pwm.png)

   2. CAPTURE_0 CCU4 slice configured in capture mode. The capture input signal is connected to the output of the PWM_0, i.e. both use the same pin. It will capture the timer value on the rising and falling edges of the generated PWM signal above.
...
The CYBSP_DEBUG_UART and PWM_0 and CAPTURE_0 resources are not enabled in the default board configuration provided by the BSP. This example overrides the `BSP_DESIGN_MODUS` component provided with the BSP to enable and configure  CYBSP_DEBUG_UART. See the "Overriding the BSP Configuration Files" section of the [ModusToolbox User Guide](http://www.cypress.com/ModusToolboxUserGuide): *{ModusToolbox install directory}/ide_{version}/docs/mtb_user_guide.pdf*
